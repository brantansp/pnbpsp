...
public class BasePage extends ObjectRepository {
	...
	public void addBankForAccNo(String bankName, String accNo) throws InterruptedException {
		...
		/*
		 * if(waitForElement (yesBtn, 50)) { String [] text= loadTextView();
		 * log.info(text[0]);
		 * if("ACCOUNT DOES NOT EXIST (REMITTER)".equals(text[0])) {
		 * log.info(text[0]); }
		 * 
		 * Integer x=1; while
		 * ("Your last transaction was not processed due to a connectivity problem. Please retry after sometime. If the problem persists, then please contact your app provider."
		 * .equals(text[0])) { log.info("Transaction was time out"); while
		 * (waitForElement (yesBtn, 50)) {
		 * log.info("Transaction timed out and found OK : " +x); click(yesBtn);
		 * click(searchBankBox); coords =getxy(searchBankBox);
		 * TapinBankName(coords, 100, 100); x++; if(x-3==0) {
		 * log.info("Breaking inner loop"); break; } } if(x-3==0) {
		 * log.info("Breaking outer loop"); break; } } }
		 */
		/*
		 * if(waitForElement (yesBtn, 50)) { click(yesBtn); back(); }
		 */
	}
	...
	public void CollectMoney(String vpa, String amt, String date, String hour, String minute, String meridian,
			String remark) {
		...
		click(fromAccSpinner);
		/*
		 * String [] options = loadTextView(); clickTextView(options[1]);
		 */
		clickRelativeLayout("1");
		waitForElement(fromAccVPA, 30);
		click(fromAccVPA);
		/*
		 * sleep(5000); String [] vpalist = loadTextView();
